...
namespace Tabacu {
	...
	void GameOverState::Init() {
		...
		_scoreText.setPosition(_data->window.getSize().x / 10 * 7.25, _data->window.getSize().y / 2.25);
		...
		_highScoreText.setPosition(_data->window.getSize().x / 10 * 7.25, _data->window.getSize().y / 1.88);
	}
	...
	int GameOverState::getHighScore() {
	...
	/*static void GameOverState::startScoring() {
		std::ifstream readFile;
		readFile.open(HIGHSCORE_FILEPATH);
